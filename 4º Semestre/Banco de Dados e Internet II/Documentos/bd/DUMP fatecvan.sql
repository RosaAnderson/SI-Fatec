...
insert  into `categorias`(`id_categoria`,`descritivo`) values (1,'Moda'),(2,'Cama-Mesa-Banho'),(3,'Eletro'),(4,'Eletrônicos'),(5,'Utilidades domésticas'),(6,'Automotivos'),(7,'teste');
...
insert  into `clientes`(`id_cliente`,`nome`,`sexo`,`datanasc`,`logradouro`,`bairro`,`cidade`) values (1,'Lucas','M','1994-02-01','Rua Vitoria 1900','Jardim das Flores','São Carlos'),(2,'Gabriel','M','1996-03-02','Rua das flores 2900','Centro','Araraquara'),(3,'Wdson','M','1998-04-03','Rua das rosas 3900','Jd Bandeirantes','Jahu'),(4,'Gilmar','M','1990-05-04','Rua Sucesso 4900','Parque das Arvores','Araraquara'),(5,'Patricia','F','1991-06-05','Rua sem saida 5900','Centro','Bauru'),(6,'Beatriz','F','1984-07-06','Av bandeirantes 6900','Planalto','Ibate'),(7,'Adriana','F','1989-08-07','Av Miguel Petroni 7900','Santa Marta','São Carlos'),(8,'Isabela','F','1991-09-08','Rua Tirandentes 900','Queto','Bocaina'),(9,'Carla','F','1993-02-10','Rua Delta 49','Palmares','Americo'),(10,'Camila','F','1992-11-21','Rua Pele 1350','Centro','Brotas');
...
insert  into `itens_nfv`(`id_infv`,`id_nfv`,`id_produto`,`quantidade`,`pvenda`) values (1,1,1,1,'45.00'),(2,1,4,1,'45.00'),(3,1,3,1,'35.00'),(4,2,3,1,'75.00'),(5,2,4,1,'30.00'),(6,2,6,1,'40.00'),(7,3,7,1,'60.00'),(8,3,8,1,'52.50'),(9,3,9,1,'30.00'),(10,4,10,1,'37.50'),(11,4,11,1,'45.00'),(12,5,12,1,'52.50'),(13,5,13,1,'52.50');
...
insert  into `nf_vendas`(`id_nfv`,`emissao`,`valor`,`id_fp`,`id_vendedor`,`id_cliente`) values (1,'2023-01-02','125.00',1,1,1),(2,'2023-01-03','145.00',2,1,1),(3,'2023-01-04','142.50',1,3,2),(4,'2023-01-05','82.50',2,4,3),(5,'2023-01-06','220.00',2,5,4),(6,'2024-03-10','100.00',1,1,1);
...
insert  into `vendedores`(`id_vendedor`,`nome`,`sexo`,`comissao`,`telefone`,`salario_fixo`) values (1,'José','M',10,'16991271861','1800.00'),(2,'Carlos','M',10,'16992272862','1800.00'),(3,'Antonio','M',10,'16993273863','1800.00'),(4,'Maria','F',15,'16994274864','1800.00'),(5,'João','M',10,'16995275865','1800.00'),(6,'Pedro','M',10,'16995276866','1800.00'),(7,'Henrique','M',10,'16997277867','1800.00'),(8,'Roberta','F',15,'16992287868','1800.00'),(9,'Maria','F',10,'16992268765','1500.00'),(10,'Tonho','M',10,'16992259876','1700.00'),(11,'Tonho','M',10,'16992259876','1700.00');
...
begin
   ## corpo procedure
   if (codc in (select id_categoria from categorias)) then # verificar o cadastro da categoria
     if (codc in (select id_categoria from produtos)) then # verifivar se a categ tem produtos
       update produtos set venda = venda + (venda*perc/100)
       where codc = id_categoria;
     else # categ não possui produtos
      select "Categoria não possui produtos" as msg;
     end if;
   else ## categoria não cadastrada
    select "Categoria não cadastrada" as msg;
   end if;
end */$$
DELIMITER ;
...
begin
   ...
   if (codp in (select id_produto from produtos)) then
    ...
    ifnull(sum(iv.quantidade*iv.pvenda),0) "TotalVendido"
    ...
    select concat("Produto ",codp," não localizado") as msg;
   end if;
end */$$
DELIMITER ;
