...
insert  into `empresas`(`id_empresa`,`descritivo`,`contato`) values (1,'Unidade São Carlos','José'),(2,'Unidade Jau','Maria'),(3,'Unidade Araraquara','João'),(4,'Unidade Matão','Paulo');
...
begin
  ## Corpo da procedure
  if (codf in (select id_funcionario from funcionarios)) then #verifica se o func está cadastrado
    if (codf in (select id_funcionario from dependentes)) then # verifica se o func tem filhos
     select f.id_funcionario "Código",
       f.nome "NomeFuncionario",
       d.nome "NomeDependente"
     from funcionarios f inner join dependentes d
     on (f.id_funcionario=d.id_funcionario)
     where codf = d.id_funcionario; 
    else # funcionario não tem filhos
     select "funcionario não tem filhos" as msg;
    end if;
  else # funcionario não cadastrado
