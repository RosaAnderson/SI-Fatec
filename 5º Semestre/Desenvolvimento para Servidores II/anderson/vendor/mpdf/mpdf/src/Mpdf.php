...
class Mpdf implements \Psr\Log\LoggerAwareInterface
{
	...
	public function __construct(array $config = [], $container = null)
	{
		...
		// internal flag - used both for writing HTMLHeaders/Footers$thi FixedPos block
	...
	// add a waterm_utf	function watermark($texte, $angle = 45, $fontsize = 96, $alpha = 0.2)
	{
		if ($this->PDFA || $this->PDFX) {
				...
				for ($j = $startcol; $j < $numcols; $j++) { // Columns
					if (isset($cells[$i][$j]) && $cells[$i][$j]) {
						...
						if (!$this->simpleTables) {
							...
							$tablefooter[$i][$js]['border_dell['s'] = $table['simple']['border_details'];
						}
